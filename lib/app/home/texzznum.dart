...
class _TexPageState extends State<TexPage> {
  ...
  void initState() {
    ...
    var check = checkIfNeedOpen();

    Future.wait([check]).then((value) {
  ...
  Widget build(BuildContext context) {
    return WillPopScope(
      onWillPop: () {
        if (widget.data.redirect) {
          Navigator.push(context, MaterialPageRoute(builder: (context) => const HomePage(isRedirect: false)));
        } else {
          Navigator.pop(context);
        }
